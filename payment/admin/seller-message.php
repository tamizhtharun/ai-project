					...
					<div class="box-body">
						<div class="form-group">
							...
							<div class="col-sm-4">
								...
								<?php
								$statement = $pdo->prepare("SELECT * FROM tbl_seller_registration ORDER BY seller_id ASC");
								$statement->execute(array($_REQUEST['id']));
								$statement->rowCount();
								$result = $statement->fetchAll(PDO::FETCH_ASSOC);
								foreach ($result as $row) {
									?>
									<option value="<?php echo $row['seller_id']; ?>"><?php echo $row['seller_name']; ?> - <?php echo $row['seller_email']; ?></option>
									<?php
								}
								?>
								</select>
